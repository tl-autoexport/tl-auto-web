...
export function MobileCatalogExperience({ currentQuery, options, sortOptions, totalCars }: {
  ...
  function reset() {
    ...
    setDraft((previous) => {
      ...
      if (["parameters", "year", "price"].includes(screen)) PARAM_KEYS.forEach((key) => next.delete(key));
  ...
  return <div className="md:hidden">
    <div className="grid gap-2">
      <button className="flex min-h-14 items-center justify-between rounded-xl border border-[#d7dee8] bg-white px-4 text-left" onClick={() => setScreen(selected("brand") ? "parameters" : "brand")} type="button">
        <span><span className="block text-[10px] font-semibold uppercase tracking-[.12em] text-[#956f2c]">Фильтр</span><span className="mt-0.5 block text-[15px] font-semibold text-[#101827]">{selectedSummary || "Марка и модель"}</span></span><ChevronRight className="text-[#647084]" size={20} />
      </button>
...
function Picker({ axis, facets, find, inputRef, loading, onChoose, onFind, selectedValue }: { axis: "brand" | "model" | "generation"; facets: Facets | null; find: string; inputRef: React.RefObject<HTMLInputElement | null>; loading: boolean; onChoose: (axis: "brand" | "model" | "generation", item: Option) => void; onFind: (text: string) => void; selectedValue: string }) {
  ...
  return <><label className="relative mb-4 block"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#7a8798]" size={20} /><input className="h-13 w-full rounded-xl bg-white pl-11 pr-4 text-base outline-none ring-1 ring-[#e0e5ec] focus:ring-[#a98239]" onChange={(event) => onFind(event.target.value)} placeholder={`Поиск: ${titleFor(axis).toLowerCase()}`} ref={inputRef} value={find} /></label><div className="overflow-hidden rounded-2xl bg-white">{loading && !facets ? <p className="p-5 text-sm text-[#647084]">Загружаем варианты…</p> : items.length ? items.map((item) => { const isSelected = selectedValue === item.value; return <button className={`flex min-h-14 w-full items-center gap-3 border-b border-[#edf0f4] px-4 text-left ${isSelected ? "bg-[#fbf7ed]" : ""}`} key={item.value} onClick={() => { onFind(""); onChoose(axis, item); }} type="button"><span className="min-w-0 flex-1 truncate text-[15px] font-medium">{item.label}</span><span className="text-xs text-[#7a8798]">{item.cars}</span><span aria-hidden="true" className={`grid size-6 place-items-center rounded-md border ${isSelected ? "border-[#a98239] bg-[#a98239] text-white" : "border-[#b9c1cb] text-transparent"}`}>✓</span></button>; }) : <p className="p-5 text-sm text-[#647084]">Нет вариантов для текущего отбора.</p>}</div></>;
}

function Parameters({ generationLabel, onSelectLevel, options, patch, selected }: { generationLabel: string; onSelectLevel: (screen: Screen) => void; options: FieldOptions; patch: (values: Record<string, string | null>) => void; selected: (name: string) => string }) {
