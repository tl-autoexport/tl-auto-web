...
export function PrototypeVehicleCard({ car }: { car: CatalogCar }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between border-y border-[#d8eadc] bg-[#f6fbf7] px-2 text-[11px] font-semibold text-[#237a3b] sm:px-3 sm:text-xs">
        <a aria-label={`Получить консультацию по автомобилю ${title}`} className="my-2 inline-flex min-h-9 min-w-0 items-center justify-center gap-1.5 rounded-full bg-[#39a852] px-3 py-2 text-white shadow-sm transition hover:bg-[#2f8f45] sm:min-h-10 sm:px-4" href={whatsappContactUrl(message)} onClick={(event) => event.stopPropagation()} rel="noreferrer" target="_blank"><MessageCircle size={16} /><span className="truncate">Консультация</span></a>
        <button aria-label={favorite ? "Убрать из избранного" : "Добавить в избранное"} className="grid size-10 place-items-center border-l border-[#d8eadc] transition hover:bg-[#e8f5eb] sm:size-12" onClick={(event) => { event.stopPropagation(); setFavorite((current) => !current); }} title={favorite ? "Убрать из избранного" : "Добавить в избранное"} type="button"><Heart fill={favorite ? "currentColor" : "none"} size={17} /><span className="sr-only">{favorite ? "Убрать из избранного" : "Добавить в избранное"}</span></button>
        <button aria-label="Поделиться объявлением" className="grid size-10 place-items-center border-l border-[#d8eadc] transition hover:bg-[#e8f5eb] sm:size-12" onClick={(event) => { event.stopPropagation(); void share(); }} title="Поделиться объявлением" type="button"><Share2 size={17} /><span className="sr-only">Поделиться объявлением</span></button>
      </div>
