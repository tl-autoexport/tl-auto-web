...
export function PrototypeVehicleCard({ car }: { car: CatalogCar }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 grid grid-cols-[minmax(0,1fr)_auto_auto] border-y border-[#d8eadc] bg-[#f6fbf7] text-[11px] font-semibold text-[#237a3b] sm:text-xs">
        <a aria-label={`Получить консультацию по автомобилю ${title}`} className="flex min-h-10 min-w-0 items-center justify-center gap-1.5 bg-[#39a852] px-2.5 py-2 text-white transition hover:bg-[#2f8f45] sm:min-h-12 sm:px-3" href={whatsappContactUrl(message)} onClick={(event) => event.stopPropagation()} rel="noreferrer" target="_blank"><MessageCircle size={16} /><span className="truncate">Консультация</span></a>
