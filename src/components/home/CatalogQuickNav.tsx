...
export function CatalogQuickNav() {
  ...
  return (
    <nav
      ...
      className="sticky top-[var(--site-header-mobile-offset,112px)] z-40 isolate border-b border-[#dce2eb] bg-white/95 shadow-[0_5px_12px_rgba(15,31,49,0.06)] backdrop-blur transition-[top] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] sm:top-[var(--site-header-mobile-offset-sm,118px)] lg:top-[76px]"
    >
      <div className="mx-auto max-w-7xl px-4 py-2.5 sm:px-5">
        <div className="scrollbar-none flex items-center gap-2 overflow-x-auto">
          ...
          <QuickButton label="Корея" onClick={() => toggleMenu("country")} open={openMenu === "country"} />
        ...
        {openMenu ? (
          <div className="relative">
            <div className="mt-2 rounded-2xl border border-[#dce2eb] bg-white p-4 shadow-[0_12px_30px_rgba(16,24,39,0.12)] sm:absolute sm:left-0 sm:top-0 sm:min-w-[360px]" role="dialog" aria-label="Настройки каталога">
              ...
              {openMenu === "country" ? <CountryPanel /> : null}
...
function CountryPanel() {
  return <div><PanelTitle>Страна и источник</PanelTitle><p className="mt-2 text-sm leading-5 text-[#647084]">Сейчас в витрине доступны объявления из Южной Кореи.</p><Link className="mt-4 flex items-center justify-between rounded-md bg-[#f5f0e4] px-3 py-3 text-sm font-semibold text-[#5c4317]" href="/catalog"><span>Южная Корея</span><span className="text-xs">Выбрано</span></Link></div>;
}
