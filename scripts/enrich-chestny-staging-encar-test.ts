...
const delayMs = Math.max(1_000, Number(process.env.CHESTNY_STAGING_ENCAR_DELAY_MS ?? 3_000));
...
async function main() {
  ...
  const { data: candidateRows, error } = await db.from("chestny_catalog_staging").select("source_listing_id,source_url,image_urls,raw_payload,fuel_type,exterior_color").eq("source_status", "active").eq("promotion_status", "auto_candidate").order("source_updated_at", { ascending: false }).limit(Math.min(1000, limit * 5));
  if (error) throw error;
  const data = (candidateRows ?? []).filter((row) => !obj(obj(row.raw_payload).encar_enrichment).fetched_at).slice(0, limit);
