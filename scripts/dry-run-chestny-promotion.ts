...
async function main() {
 ...
 try {
  const [staging, refs] = await Promise.all([
   c.query(`select manufacturer,model,generation,trim,model_year,engine_cc,fuel_type,drive_type from public.chestny_catalog_staging`),
