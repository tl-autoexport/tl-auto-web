...
const alias: Record<string, string> = { avante: "Elantra", canival: "Carnival", tiboli: "Tivoli", "1-series": "1 Series", "2-series": "2 Series" };
const model = (v: unknown) => alias[String(normalizeModel(v) ?? "").toLowerCase()] ?? normalizeModel(v);
  ...
  try {
    const [rows, refs] = await Promise.all([
      db.query(`select s.source_listing_id,s.manufacturer,s.model,s.generation,s.trim,s.model_year,s.engine_cc,s.fuel_type,s.drive_type
        ...
        where q.status='succeeded' and s.source_status='active' and c.id is null`, [runId]),
    ...
    for (const r of rows.rows) {
      const result = resolveApprovedPower({ brand: normalizeBrand(r.manufacturer), model: model(r.model), generation: r.generation, trim: r.trim, badge: r.trim,
        year: r.model_year, engineCc: r.engine_cc, fuelType: normalizeFuel(r.fuel_type), driveType: normalizeDrive(r.drive_type) }, candidates);
      ...
      try {
        for (const item of matched) {
          const result = await db.query(`update public.chestny_catalog_staging set promotion_status='power_confirmed', promotion_note=$2, updated_at=now()
            where source_listing_id=$1 and source_status='active' and promotion_status <> 'published'`,
    ...
    console.log(JSON.stringify({ runId, reserveCards: rows.rowCount, approvedReferenceMatches: matched.length, reviewRequired: unresolved.length,
      ...
      sampleUnresolved: unresolved.slice(0, 20), encarRequests: 0, publicCatalogChanged: false }, null, 2));
