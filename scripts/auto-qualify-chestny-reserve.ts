...
async function main() {
  ...
  try {
    const [staging, refs, active] = await Promise.all([
      c.query<StagingRow>(`select source_listing_id,source_url,source_status,manufacturer,model,model_year,mileage_km,price_krw,engine_cc,fuel_type,transmission,drive_type,exterior_color,image_urls,raw_payload,promotion_status,last_seen_at from public.chestny_catalog_staging where source_status='active' and promotion_status not in ('published','rejected')`),
