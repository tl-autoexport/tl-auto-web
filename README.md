...
## Обновление каталога и актуальность

Ежедневный GitHub Actions sync отключён: GitHub больше не обращается к Encar
автоматически, потому что такой трафик блокируется источником. Workflow
`.github/workflows/catalog-sync.yml` оставлен только для явного ручного запуска
через `workflow_dispatch` (dry-run или write) после согласования.

Автоматическая проверка уже опубликованных объявлений — отдельный VPS-процесс
с прокси (`check:encar:queue:write`). Он читает только активные записи TL Auto
из `chestny_prigon`, проверяет их небольшими партиями и не импортирует новый
каталог Encar. Это не GitHub sync и не обращается к базе «Честного пригона».
